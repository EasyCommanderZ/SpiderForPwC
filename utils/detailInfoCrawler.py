...
class detailInfoCrawler:
    ...
    def getDetailInfo(self, url):
        ...
        abstract = soup.find("blockquote", attrs={'class': "abstract"}).text[12:]
        year = soup.find("div", attrs={'class': "dateline"}).text
        pdfUrl = "https://arxiv.org" + soup.find("a", attrs={'class': "download-pdf"})['href'] + '.pdf'

        extra_content = requests.get("https://arxiv.paperswithcode.com/api/v0/papers/" + url.split('/')[-1]).content
